...
loss2                 = sum(sum((T*x_s - clean).^2))/R;

psz                   = sqrt(size(clean,1));
bsz                   = TP.fsz + 1;
x_s2                  = T*x_s;
lossvec               = zeros(R,1);
grad_l_uori           = zeros(size(clean));
parfor i = 1:R
    xi                = reshape(x_s2(:,i), psz, psz);
    cleani            = reshape(clean(:,i), psz, psz);
    [mssim, d_loss_x] = ssim_index(xi, cleani, bsz, [psz,psz]);
    lossvec(i)        = 1-mssim;
    grad_l_uori(:,i)  = -d_loss_x;
end
loss                  = sum(lossvec) / R;
fprintf('Iter: %03d\tLoss: %.6f\tLoss-l2: %.6f\t', TP.iter, loss, loss2);
%% derivative of loss wrt x
AllNetWorks{stage}.grad_l_u  = 1/R*T'*grad_l_uori;
