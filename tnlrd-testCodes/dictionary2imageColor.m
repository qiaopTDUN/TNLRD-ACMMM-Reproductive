function I = dictionary2imageColor(D, nlK, color)
    [ksz, N] = size(D);
    ksz = sqrt(ksz);
    bsz = 40;
    nc = 12;
    ...
    for i = 1:nc
        for j = 1:nr
            ...
            for ii = 1:ksz
                for jj = 1:ksz
                    ind = int16(si(jj,ii)*256);
                    ind = min(max(ind, 1), 256);
                    clr = color(ind,:);
            ...
            for jj = 1:length(si)
                transi = 1-(si(jj)-1)^4;
                ind = int16(transi*256);
                ind = min(max(ind, 1), 256);
                clr = color(ind,:);
                I((bsz+ksz*scale)*(j-1)+bsz+1+ksz*scale+dshift:(ksz*scale+bsz)*(j-1)+ksz*scale+bsz+dshift+scale,(bsz+ksz*scale)*(i-1)+bsz+1+(jj-1)*scale+lshift:(ksz*scale+bsz)*(i-1)+jj*scale+bsz+lshift,1) = clr(1);
                I((bsz+ksz*scale)*(j-1)+bsz+1+ksz*scale+dshift:(ksz*scale+bsz)*(j-1)+ksz*scale+bsz+dshift+scale,(bsz+ksz*scale)*(i-1)+bsz+1+(jj-1)*scale+lshift:(ksz*scale+bsz)*(i-1)+jj*scale+bsz+lshift,2) = clr(2);
                I((bsz+ksz*scale)*(j-1)+bsz+1+ksz*scale+dshift:(ksz*scale+bsz)*(j-1)+ksz*scale+bsz+dshift+scale,(bsz+ksz*scale)*(i-1)+bsz+1+(jj-1)*scale+lshift:(ksz*scale+bsz)*(i-1)+jj*scale+bsz+lshift,3) = clr(3);
            end
